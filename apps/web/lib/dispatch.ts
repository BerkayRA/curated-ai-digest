 ...
 *
 * Designed for injection of provider + repo so unit tests can mock both.
 * Reused by Phase 10 worker (just pass the production prisma + provider).
 *
 * Contract for Phase 10:
 *   import { dispatchIssue } from '@mega-bulten/web/lib/dispatch';
 *   await dispatchIssue(issueId);
 *
 * Or with a custom provider for testing:
 *   await dispatchIssue(issueId, { provider: mockProvider, repo: mockRepo });
 */
...
const defaultRepo: DispatchRepo = {
 ...
 *  3. Render email once (with a placeholder unsubscribeUrl for the HTML)
 *  4. For each subscriber, build a personalised EmailMessage
 *  5. sendBatch via provider
 *  6. Record Send rows
 *  7. Transition issue to 'sent' (or 'failed' if all sends failed)
 */
