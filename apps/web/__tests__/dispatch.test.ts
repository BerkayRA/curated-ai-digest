 ...
 * Uses injected mock provider + mock repo — no real DB or email calls.
 */
...
// ---------------------------------------------------------------------------
// Mock setup — vi.mock must be hoisted
// ---------------------------------------------------------------------------

vi.mock('../lib/issue-transition', () => ({
  transitionIssue: vi.fn().mockResolvedValue({ id: 'issue-1', status: 'sent' }),
}));

// Import after mocks are set up
const { dispatchIssue } = await import('../lib/dispatch');
const { transitionIssue } = await import('../lib/issue-transition');
...
describe('dispatchIssue', () => {
  ...
  it('sends to all active subscribers and records Send rows', async () => {
    ...
    const result = await dispatchIssue('issue-1', { provider, repo });
  ...
  it('builds messages with correct unsubscribe URL and List-Unsubscribe header', async () => {
    ...
    await dispatchIssue('issue-1', { provider, repo });
  ...
  it('transitions to failed and records error Send rows when provider throws', async () => {
    ...
    vi.mocked(transitionIssue).mockResolvedValueOnce({ id: 'issue-1', status: 'failed' });
  ...
  it('throws when provider verifyConfig returns not ok', async () => {
    ...
    await expect(dispatchIssue('issue-1', { provider, repo })).rejects.toThrow(/API key missing/);
  });
  ...
  it('throws when no settings row exists', async () => {
    ...
    await expect(dispatchIssue('issue-1', { provider, repo })).rejects.toThrow(/settings/i);
  });
  ...
  it('throws when issue is not found', async () => {
    ...
    await expect(dispatchIssue('issue-1', { provider, repo })).rejects.toThrow(/not found/i);
  });
  ...
  it('throws when there are no active subscribers', async () => {
    ...
    await expect(dispatchIssue('issue-1', { provider, repo })).rejects.toThrow(/subscriber/i);
  });
